...
class LinkedList:
    ...
    def jump_search(self, key, attribute):
        ...
        while current and getattr(current, attribute) < key:
            prev = current
        ...
        if current and getattr(current, attribute) == key:
            return current
        else:
            return None

    def search(self, key, attribute):
        current = self.head
        while current:
            if getattr(current, attribute) == key:
                return current
            current = current.next
        return None
...
datamajalah.append(1, "Kesehatan", 25000, 2022, "Sparkle Media")
datamajalah.append(2, "Teknologi", 30000, 2024, "Green Planet")
datamajalah.append(3, "Travel", 18000, 2023, "Travelogue Ltd")
datamajalah.append(4, "Lingkungan", 20000, 2024, "Green Planet")
...
def searchitem():
    ...
    print("")
    pilihan_search = int(input("PILIHAN SEARCHING (1/2) : "))

    if pilihan_search == 1:
        harga = int(input("\nMASUKKAN HARGA MAJALAH: "))
        result = datamajalah.search(harga, 'harga')
    elif pilihan_search == 2:
        tahun_terbit = int(input("\nMASUKKAN TAHUN TERBIT MAJALAH: "))
        result = datamajalah.search(tahun_terbit, 'tahun_terbit')
    else:
        print("\nPILIHAN TIDAK VALID")
        pilihan_menu()
        return
    ...
    if result:
        print("\nDATA DITEMUKAN:\n")
        print("Nomor:", result.id, '| Jenis:', result.jenis, '| Harga:', result.harga, '| Tahun Terbit:', result.tahun_terbit, "| Penerbit:", result.penerbit)
    else:
...
def pilihan_menu():
    while True:
        ...
        print("7. KELUAR")
        print('')
        pilihan_menu = int(input("PILIHAN MENU (1/2/3/4/5/6/7) : "))
